...
until response == number do #keep running program until the response matches the number (which is random)
	if response < number then puts 'Too Low!/nTry again!' #if the response is lower than the number. Output is too low.
		elsif response > number then puts 'Too High!/nTry again!'#if the response is higher than the number. Ouput is too high.
	end
